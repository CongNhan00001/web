...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		if("ログイン".equals(btn)) {
			...
			response.sendRedirect("./WEB-sample/login.jsp");
			...
			rd = request.getRequestDispatcher("./WEB-sample/history.jsp");
			rd.forward(request, response);
			
		}else if("新人登録".equals(btn)) {
			
			response.sendRedirect("./WEB-sample/input.jsp");
			
		}else if("送信".equals(btn)) {
			...
			try {
				login.updateUser(id, pass, name, Integer.parseInt(age));
				int r=login.r;
				if(r!=0) {
					response.sendRedirect("./WEB-sample/usenew.jsp");
				}else {
					response.sendRedirect("./WEB-sample/usenew.jsp");
				}
