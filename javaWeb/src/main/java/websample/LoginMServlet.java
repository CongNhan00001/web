...
public class LoginMServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		if("ログイン".equals(btn)) {
			...
			LoginMa login = new LoginMa();
			
			// ③-1-4 ID処理クラスに②-1-1で取得したIDを渡してユーザ情報をモデルに格納
			
			LoginUB bean=login.getUserData(id, pass);
			// ③-2 モデルの情報を判定
			if(bean != null) {
				...
				rd = request.getRequestDispatcher("./WEB-sample/top.jsp");
				
			} else {
				
				// ③-3 モデルの情報が存在しない（IDに紐づくユーザ情報がない）場合
				// ③-3-1 つぎに表示させる画面（ビュー）を指定
				rd = request.getRequestDispatcher("./WEB-sample/erro.jsp");
			}
			...
			response.sendRedirect("./WEB-sample12/loginMa.jsp");
		}
	}
}